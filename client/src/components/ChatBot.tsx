...
const ChatBot = () => {
  ...
  return (
    <div className={`fixed bottom-4 right-4 z-50 flex flex-col items-end transition-all duration-300 ease-in-out ${isExpanded ? 'w-full sm:w-96 h-[500px]' : 'w-auto'}`}>
      {/* Main chat container */}
      <motion.div 
        layout
        ...
        transition={{ duration: 0.3 }}
        className={`bg-white dark:bg-slate-800 rounded-2xl shadow-xl overflow-hidden flex flex-col ${isExpanded ? 'w-full' : 'max-w-xs'}`}
      >
        {/* Chat header */}
        <motion.div 
          layout
