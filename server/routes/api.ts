...
router.post('/articles/:slug/helpful', async (req: Request, res: Response) => {
  try {
    ...
    log(`Error updating article helpful votes: ${error}`, 'api');
    return res.status(500).json({ error: 'Internal server error' });
  }
});

router.post('/news/:slug/helpful', async (req: Request, res: Response) => {
  try {
    const { slug } = req.params;
    const { helpful } = req.body;
    ...
    log(`Error updating news helpful votes: ${error}`, 'api');
    return res.status(500).json({ error: 'Internal server error' });
  }
});
